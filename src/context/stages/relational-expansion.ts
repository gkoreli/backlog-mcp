...
function traverseAncestors(
  ...
  visited: Set<string>,
  getTask: (id: string) => Task | undefined,
): { ancestors: Array<{ task: Task; depth: number }>; parentTask: Task | null } {
  const ancestors: Array<{ task: Task; depth: number }> = [];
...
function collectDescendants(
  ...
  visited: Set<string>,
  listTasks: (filter: { parent_id?: string; limit?: number }) => Task[],
): { children: Task[]; deepDescendants: Array<{ task: Task; depth: number }> } {
  const children: Task[] = [];
  const deepDescendants: Array<{ task: Task; depth: number }> = [];
