...
|---|---|---|---|
| Context tools — keep or cut? | Remove `backlog_context` as a tool · uplift it · fold its unique value into the existing retrieval surface | **Resolved — executed as ADR 0114 recommended (shipped 0.59.0, 2026-07-16; status reconciled 2026-07-17):** `backlog_context` and its hydration pipeline are removed; its unique value — relational neighborhood + reverse refs — folded into `backlog_get(id, context:true)` as stubs (CHANGELOG 0.59.0 Added/Removed). One retrieval language: `wakeup`=orient, `recall`/`search`=ask, `get`=expand. | onyx · ADR 0114 |
...
| Storage layout | Hidden `./.backlog` dotfolder · **open `./docs`** with per-substrate folders | **Resolved — shipped as ADR 0112 recommended (0112 family, 0.60.0–0.61.0, 2026-07-16; status reconciled 2026-07-17):** `docs/` is authoritative day-0 truth; project `.backlog/` holds only config/cache/local telemetry; control folder renamed `.backlog` and the global home flattened (ADR 0112.2/0112.3; CHANGELOG 0.61.0). Typed per-substrate folders coexist with generic unclaimed docs. | quartz · ADR 0112, 0112.2 |
| ADR modeling | ADR as an **extension of artifact** · ADR as its **own substrate** | **Resolved — live via ADR 0113 (Phases A–B shipped 0.60.0, Phase C 0.61.0, 2026-07-16; status reconciled 2026-07-17):** ADR and Requirement are **first-class flagship substrates** (native `supersedes`/lifecycle/threads); Prompt ships as the smaller provenance substrate; declared types are searchable via server-owned projections (CHANGELOG). | basalt · ADR 0113 |
| Docs home | Require a `docs/` home · **auto-explore** all `.md` and use `./.backlog` as home | **Resolved — shipped in the 0112 family (0.61.0 Phase E, 2026-07-16; status reconciled 2026-07-17):** `docs/` is the default in-project truth and the production backlog, global remains explicit; identity is filename/path-derived and substrate-neutral, threads first-class; one explicit `migrate docs-native` command (CHANGELOG 0.61.0). | quartz · ADR 0112 |
