...
| Storage layout | Hidden `./.backlog` dotfolder · **open `./docs`** with per-substrate folders | **ADR 0112 recommends (owner: Goga):** `docs/` is authoritative day-0 truth; `.backlog-mcp/` holds only config/cache/local telemetry. Typed per-substrate folders coexist with generic unclaimed docs (incl. `docs/prompts/`). | quartz · ADR 0112 |
