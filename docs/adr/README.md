...
- [0107. Loro-as-Truth — A Local-First CRDT History Substrate](./0107-loro-as-truth-local-first-history-substrate.md) **[thread root — Proposed; design-first; children 0107.1–0107.4]** - Adopt Loro (CRDT) as durable truth + history; plain markdown as a native-editable projection; capture edits as clean ops (PostToolUse hook) not lossy snapshots; multi-agent future merges via loro-protocol instead of conflict-copying. Stops reinventing VCS in the op-log. Honest core risk: no native two-way file↔Loro sync — a bidirectional bridge (incl. a Loro→file subscribe-daemon + loop-breaking) is intrinsic, justified only by the multi-agent destination - 2026-06-17
