# 0013.6. Enable StreamableHTTPServerTransport with Current Architecture

> *Threaded under [0013](./0013-http-mcp-server-architecture.md); renumbered from 0025 on 2026-06-04. Content unchanged; history via `git mv`.*
...
**ADR-0013.4** (Rejected): Attempted direct migration but failed because:
...
**ADR-0013.5** (Rejected by user): Proposed dual-mode server (separate stdio mode)
...
**Root cause of ADR-0013.4 failure**: We forced mcp-remote to use SSE (`--transport sse-only`) while the server used StreamableHTTP. Protocol mismatch.
...
- ✅ Fixes ADR-0013.4 failure root cause
...
1. **Previous failure root cause**: ADR-0013.4 changed server but not client → protocol mismatch
...
- [ADR-0013.4: Rejected Migration Attempt](./0013.4-migrate-to-streamable-http-transport.md)
- [ADR-0013.5: Rejected Dual-Mode Server](./0013.5-dual-mode-server-for-streamable-http.md)
