# 0013.3. Complete HTTP Architecture Migration

> *Threaded under [0013](./0013-http-mcp-server-architecture.md); renumbered from 0019 (the second 0019) on 2026-06-04. Content unchanged; history via `git mv`.*
...
The HTTP architecture migration (ADR-0013, ADR-0013.1) is **incomplete and broken**. We have duplicate systems running in parallel, with the new HTTP server missing critical functionality that exists in the old viewer.
