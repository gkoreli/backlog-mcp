...
**Keep `write_resource` as the agent's edit surface; swap its backend from file-writes to Loro ops.** Adopt **Loro (via `@loro-extended`)** as the durable source-of-truth and history substrate, local-first. The agent's experience is unchanged — same `str_replace`/`append`/`insert` tool, same field names, same uniqueness contract — but each edit becomes a clean Loro operation. **There is NO file bridge, NO hook, NO watcher, NO Loro→file daemon, NO loop-breaking.** Those were all artifacts of "agent edits a file, sync it to the doc." The resolution: the agent never edits a file — it calls a tool, and *the tool call is the op-capture point*. Markdown is a **render-on-read** (`LoroText.toString()`), never a persisted editable surface.

### The central reframe (why the bridge evaporated)
An agent cannot natively edit a CRDT — its I/O is bytes, the CRDT consumes ops. A **translation tax** is therefore unavoidable; the only question is *where it lives*:
- **In a file bridge** (hook/watcher → ops): keeps a literal file, but owns the hardest, most bug-prone machinery (bidirectional sync, loop-breaking, `Loro→file` daemon). Rejected.
- **In the edit tool** (`write_resource` → ops): the agent calls a tool that already speaks its native edit vocabulary; the tool resolves the anchor and emits ops directly. **Selected** — the tax is paid once, in code we already own, with zero file-sync machinery.
- *(Electric `collaborative-ai-editor` validates this: their agent is a CRDT peer via constrained document tools — `search_text`→stable handle→`replace_matches` — never editing files. Our `write_resource` is the same model in the agent's existing vocabulary.)*

### Why this is NOT the `write_resource` regret repeated
The original `write_resource` was "detrimental" because it replaced native editing **and bought nothing** (it just wrote a file, worse than native `Edit`). The CRDT-backed `write_resource` keeps the *same surface* but **buys the entire CRDT** — history, time-travel, attribution, a no-rewrite path to multi-agent merge — capabilities native file `Edit` cannot provide. A custom tool that earns its existence is not the mistake.

### Native-feel design constraints (the load-bearing details)
- **Mirror the native edit-tool shape exactly** so the driving model stays *in its trained distribution* (measurably better edit quality, not aesthetics). Copy Claude's `Edit` contract field-for-field: `old_string` / `new_string` / `replace_all`, with the **unique-or-reject** rule (`old_string` must match exactly and uniquely). `old_string` uniqueness IS Electric's "stable handle."
- **`get` renders markdown** (`LoroText.toString()`) so the agent reads current text to choose anchors. No file involved either direction — doc is the only truth; text is a render on read, ops on write.
- **Anchored multi-line replaces reduce to minimal line-level ops** (via `updateByLine`-style internal diff), **never** block `delete+insert` — preserves unchanged lines' CRDT node identity (correct history + concurrent-safe).
- **No fuzzy matching — exact-or-reject.** Misapplying an edit at the wrong CRDT position is silent corruption; native harnesses fuzz, we must not.
- **(Deferred) diff-hunk dialect**: optionally accept `apply_patch`-shaped input later for GPT/Codex-family fluency. Same internal `ReplaceOp`. Not needed for the single-driver MVP; noted so the door stays open.

### Execution as the 0107 thread
- **0107.1 — Domain mapping**: entity → Loro containers via **`@loro-extended/change`** typed schemas. `LoroMap` (frontmatter, LWW), `LoroText` (body), `LoroTree` (`parent_id` hierarchy). Substrate Zod stays the **app-layer validity gate** (CRDTs don't validate at write time).
- **0107.2 — CRDT-backed `write_resource`**: re-point `str_replace`/`append`/`insert` from `FilesystemStorage` writes to Loro ops; `get` renders `LoroText.toString()`. Mirror Claude's `Edit` contract; exact-match-or-reject; minimal line-level ops. This *replaces* the earlier "hook/watcher op-capture" plan — the tool is the capture point.
- **0107.3 — Storage**: persist the Loro doc as a snapshot blob behind the 0106.3 `StorageAdapter` seam — proven by pitlane (`export({mode:'snapshot'})` per doc, debounced save on `subscribe`). The readable `.md` is an **optional projected export** (for git/human/grep), NOT a synced editable surface — so no bridge, no loop. (Defer the projection if not needed day one; the blob is the truth.)
- **0107.4 — Sync (future)**: agents as **peers** via `@loro-extended`'s websocket/webrtc/sse adapters. Merge, not conflict-copy. **Not built until concurrent multi-agent writing is live** — substrate choice keeps the door open with no rewrite.
...
- **The file↔Loro bridge is AVOIDED by design (corrected from an earlier draft).** An earlier draft of this ADR called a bidirectional file bridge "intrinsic and unavoidable." That was true *only under the assumption that the agent edits a plain file*. The decision drops that assumption: the agent edits via `write_resource` (a tool), so **the tool call is the op-capture point** — there is no file to watch, no `Loro→file` daemon, no loop-breaking. The translation tax moved from a bridge into the edit tool. **The readable `.md` becomes an optional one-way *export* (Loro→file, for git/human/grep), never a synced editable input** — one-way projection has no loop and no conflict surface. *(The bridge cost was real; we eliminated it by changing where editing happens, not by wishing it away.)*
- **Native-feel / edit-quality risk.** An MCP tool can match the native edit-tool *shape* but the model still knows it's a named tool, not its built-in `Edit` (can't replicate harness-level Read-precondition, streaming, retries). Mitigation: mirror Claude's `Edit` contract field-for-field (`old_string`/`new_string`/`replace_all`, unique-or-reject) + describe in the models' own vocabulary → stay in the trained distribution. 100%-native is only reachable via FUSE (considered, rejected: kernel daemon, platform-specific, and `Edit`-rewrites-whole-file means no fidelity gain).
- **Multi-line anchor staleness.** Larger `old_string` blocks fail exact-match more often (drift between read and write). Mitigation: encourage smaller anchors in the tool description; exact-or-reject (never fuzzy — CRDT misapplication is silent corruption); stable Loro cursors (`getCursor`) as a later robustness upgrade.
- **Validity outside the merge.** CRDTs don't enforce write-time invariants; the substrate Zod parse must run app-side on every op-apply (0107.1/0107.2). A real seam, not free.
- **Binary truth-store.** Truth is the Loro snapshot blob, not the `.md`. Mitigation: optional one-way markdown export keeps a readable/git-committable artifact; the manual EOD git habit becomes optional, not broken. (pitlane accepts the same tradeoff, mitigated by `exportForTraining`.)
- **Assembly cost.** `@loro-extended` supplies repo/sync/storage/schema; pitlane proves the persistence pattern. The net-new code is the CRDT-backed `write_resource` op-translation (~tens of lines) + the `change` schema mapping. Small, fits the 0106.3 `StorageAdapter` seam.

## Consequences

- **Positive**: one history substrate; **`write_resource` surface unchanged for the agent** (no re-education, native feel); real diffs/time-travel/revert/attribution; **no file bridge / hook / watcher / loop** (eliminated by making the tool the op-capture point); the op-log shrinks to an event index; a no-rewrite path to concurrent multi-agent; `LoroTree` fits the domain; fits the 0106.3 seam; substrate + persistence patterns proven by pitlane.
- **Negative**: binary truth (mitigated by optional one-way markdown export); validity enforcement moves app-side (Zod on op-apply); a CRDT-backed op-translation to write and harden (small, but exact-match-or-reject correctness is load-bearing); new dependency surface (`@loro-extended` — already used in another operator project).
- **Supersedes the premise of**: ADR 0089 (Cloudflare Workers + D1 as a *write* backend — demotes to sync target / read-replica or out of scope under local-first) and the remote-first premise behind `write_resource` (0001/0087) — though `write_resource`'s *tool surface* is **kept** (only its backend changes). These are premise reversals to ratify in the thread, not yet executed.

## Open Questions (resolved in children)

1. CRDT-backed `write_resource` op-translation: how to reduce an anchored multi-line `str_replace` to minimal `LoroText` line-ops (`updateByLine` internal diff vs explicit splice); exact-match-or-reject enforcement; when (if) to add stable `getCursor` handles. *(0107.2)*
2. One `LoroDoc` for the whole backlog vs one per entity vs per-folder? Doc-per-entity (pitlane's per-session model) vs tree-in-one-doc affects sync granularity, load, and `LoroTree` placement. *(0107.1, 0107.3)*
...
4. Markdown export: build the one-way Loro→file projection now (git/grep readability) or defer until needed? *(0107.3)*
5. D1/Workers: demote to read-replica, or remove? *(0107.4)*
