# 0013.2. Use mcp-remote for stdio Bridge

> *Threaded under [0013](./0013-http-mcp-server-architecture.md); renumbered from 0019 on 2026-06-04. Content unchanged; history via `git mv`.*

**Date**: 2026-01-25
**Status**: Accepted
**Supersedes**: ADR-0013.1 (implementation approach)

## Context

ADR-0013.1 implemented a custom stdio-to-HTTP bridge, but it had blocking issues during initialization. After research, we discovered `mcp-remote` - a battle-tested, MIT-licensed bridge that does exactly what we need.
...
- [0013.1. stdio-to-HTTP Bridge Implementation](./0013.1-stdio-http-bridge-implementation.md) - Superseded
