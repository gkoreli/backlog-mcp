# 0013.5. Dual-Mode Server Architecture for StreamableHTTPServerTransport

> *Threaded under [0013](./0013-http-mcp-server-architecture.md); renumbered from 0024 on 2026-06-04. Content unchanged; history via `git mv`.*

**Date**: 2026-01-25
**Status**: Rejected (Superseded by ADR-0013.6); recommendation partially superseded by ADR-0013.7
**Backlog Item**: TASK-0090

> **Partially superseded by [ADR-0013.7](./0013.7-transport-bridge-and-hosting-decision-framework.md)
> (2026-06-03).** This ADR's headline recommendation — Option 1, native in-process
> `StdioServerTransport` ("dual-mode") — is rejected for the multi-client case: N stdio
> clients each spawning an in-process server reintroduces the ID-race, divergent-index,
> and no-shared-viewer problems ADR-0013 killed. ADR-0013.7 D1 affirms the **shared daemon +
> thin bridge** model instead. Option 2 (a transparent native stdio↔HTTP bridge) remains
> viable and is folded into ADR-0013.7 D3 as a low-priority quality swap, gated by the
> ADR-0013.2 transparent-proxy constraint.

## Context

ADR-0013.4 rejected StreamableHTTPServerTransport migration due to mcp-remote incompatibility. The previous agent concluded that migration was "impossible" because mcp-remote only supports the legacy SSE protocol.
...
2. Update ADR-0013.4 status to "Superseded by ADR-0013.5"
3. Update ADR README
...
- **ADR-0013.4**: Rejected StreamableHTTP migration (superseded by this ADR)
- **ADR-0022**: Current SSEServerTransport implementation (will be replaced)
- **ADR-0013.1**: stdio-HTTP bridge implementation (will be replaced with native stdio mode)
