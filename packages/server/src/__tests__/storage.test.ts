...
describe('Storage', () => {
  ...
  describe('add', () => {
    it('should create a new task', async () => {
      const task = buildEntity({ id: 'TASK-0001', title: 'Test task' });
  ...
  describe('save', () => {
    it('should update an existing task', async () => {
      const task = buildEntity({ id: 'TASK-0001', title: 'Original' });
      await storage.add(task);
    ...
    it('should throw when saving task with invalid id', async () => {
      const task = buildEntity({ id: 'TASK-0001', title: 'Test' });
  ...
  describe('list', () => {
    it('should list only active tasks by default', async () => {
      const task1 = buildEntity({ id: 'TASK-0001', title: 'Active' });
      const task2 = buildEntity({ id: 'TASK-0002', title: 'Done' });
    ...
    it('should respect archivedLimit parameter', async () => {
      const tasks: any[] = [];
      for (let i = 1; i <= 5; i++) {
        const task = buildEntity({ id: `TASK-${String(i).padStart(4, '0')}`, title: `Task ${i}` });
        tasks.push(task);
      }
  ...
  describe('counts', () => {
    it('should return counts by status', async () => {
      const task1 = buildEntity({ id: 'TASK-0001', title: 'Open' });
      const task2 = buildEntity({ id: 'TASK-0002', title: 'Done' });
